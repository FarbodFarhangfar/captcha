...
def fully_mixed_rector(t, k, rtd):
...
result = quad(first_flotabililtis, 0, 2500, args=(1, rtd_defualt))  # first integrate

R = 0.5  # R label (actual R)
in_k = 1  # assign first K

while not R - result[0] > eps:
    result = quad(first_flotabililtis, 0, 2500, args=(in_k, rtd_defualt))
